...
Solving the frequency-domain equation for non trivial problems -- meaning ones with very large meshes, lots of frequencies or high polynomial orders -- requires the solution of a large scale linear system. Different approaches exist and are currently used when dealing with those, a recent approach, called @HDG:both @x-HDG @x-AdjointHDG, is implemented in the open-source software @HAWEN:short @x-HAWEN @x-HAWENRepo @x-HAWENWebsite @x-FloPhD. Written in Fortran and distributed under the @GPL license, it aims to provide a more scalable approach compared to traditional wave modeling programs.

The construction and factorization of the matrices that define the linear system is particularly expensive, especially in terms of memory consumption. Given that each simulation is run on an @HPC environment, researching ways to improve the efficiency of this task results in non-negligible energy savings and enables treating ever larger problems. Due to the embarrassingly parallel nature of the algorithms used in the software, taking advantage of @GPU:pl to accelerate part of the computations would be a good intuition @x-GPUHDG. In this work we will explore different ways to address the main bottlenecks in the software with a special focus on @GPU computing, in particular with NVIDIA's CUDA development platform.

== Scientific Approach and Summary of Results

After first familiarizing ourselves with the technologies used in this library and the techniques used for heterogenous computing @x-RecentMPIOpenMPGPU @x-DCvsDirectives, we will tackle the optimization problem consisting in reducing the runtime of the software by first recognizing and isolating the most expensive routines in the code. Given that the solution of the wave equation can be used for different purposes, different benchmarks are used to cover most use cases. We will notice that different ones exhibit different behavior and are affected by different routines in the code.
