...
Solving the frequency-domain equation for non trivial problems -- meaning ones with very large meshes, lots of frequencies or high polynomial orders -- requires the solution of a large scale linear system. Different approaches exist and are currently used when dealing with those, a recent approach, called @HDG:both @x-HDG @x-AdjointHDG, is implemented in the open-source software for @HAWEN:short @x-HAWEN @x-HAWENRepo @x-HAWENWebsite @x-FloPhD. Written in Fortran and distributed under the @GPL license, it and aims to provide a more scalable approach compared to traditional wave modeling programs.

The construction and factorization of the matrices that define the linear system is particularly expensive, especially in terms of memory consumption. Given that each simulation is run on an @HPC environment, researching ways to improve the efficiency of this task results in non-negligible energy savings and enables treating ever larger problems. Due to the embarrassingly parallel nature of the algorithms used in the software, taking advantage of @GPU:pl to accelerate part of the computations would be a good intuition @x-GPUHDG. In this work we will explore different ways to accelerate the main bottlenecks in the software with a particular focus on @GPU computing, in particular with NVIDIA's CUDA development platform.
