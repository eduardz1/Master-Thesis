...
#import "@preview/zero:0.4.0": format-table, num, set-round, zi
...
For the experiments that only compare different @CPU#[-only] implementations, the compiler used will be @GCC `15.1.0`. For the ones that include both a @GPU and @CPU version we will use either @GCC `13.3.0` or NVHPC `24.7`. This difference is due to some incompatibilities with newer NVHPC versions on the PlaFRIM (see @clusters) servers we are running the benchmarks on. All results are given as a mean of 10 runs. Where multiple configurations are compared against one another, we ensure that the same node is used for all the configurations for consistency.
...
To benchmark the impact of the removal of matrix inversions and the changes related to better cache locality, we use the Marmousi2 @x-Marmousi2 model to simulate the propagation of elastic waves over a mesh of 100 thousands cells and compare the difference across a set of different polynomial orders. The Marmousi2 model covers a surface of 3.5 #sym.times 17 kilometers. This benchmark has 169 sources (i.e. right-hand sides $cal(B)$ of the global linear system). In @marmousi-img we see the resulting wave field corresponding to the source number 111 at a frequency of #zi.Hz[7].
