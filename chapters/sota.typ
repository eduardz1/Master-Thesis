...
#let mark1 = markul
#let mark2 = (body, ..args) => markul(
  markul(body, stroke: .3pt, ..args),
  stroke: .3pt,
  outset: (
    bottom: -.1pt,
  ),
...
#let marc(body) = text(fill: blue, [Marc: #body])
...
$
  AA_e & = mat(
    - angle.l i omega kappa^(-1) phi_i | phi_j angle.r_K_e + tau angle.l phi_i | phi_j angle.r_(partial K_e), angle.l partial_x phi_i | phi_j angle.r_K_e, angle.l partial_y phi_i | phi_j angle.r_K_e;
    - angle.l phi_i | partial_x phi_j angle.r_K_e, - angle.l i omega rho phi_i | phi_j angle.r, 0;
    - angle.l phi_i | partial_y phi_j angle.r_K_e, 0, - angle.l i omega rho phi_i | phi_j angle.r_K_e
  ), \
  CC_e & = mat(
...
We will not discuss the @HDG method in any more detail as that would be out of scope for this work. Further reading and references can be found in the bibliography, in particular in the works of #cite(<x-AdjointHDG>, form: "prose") and #cite(<x-HDG>, form: "prose"). The performance analysis will specifically focus on the first two matrices $AA_e$ and $CC_e$ with the others being relatively inexpensive to compute. As a final remark, the key to an efficient @HDG implementation is a performant construction of the various matrices necessary to build the final system and optimized operations between the matrices.
