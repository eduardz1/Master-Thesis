...
In this section we will talk about the tools and libraries used as part of the performance work on @HAWEN.

- @clusters will be a brief introduction for the clusters used in this work.

- @par-tools will introduce the parallelism tools that the software currently takes advantage of and the ones related to @GPU computing that we explored during this work.

- @fortran-sec will be dedicated to the Fortran programming languages and the native support it provides for parallelism primitives.

- @profiling will give an overview at the current state of profiling for Fortran @MPI programs and the criteria used to choose a suitable one for this work.

- @sparse-solvers will talk about sparse linear algebra solvers and in particular the two we considered in this work.

- @compilers will briefly introduce some of the more relevant compilers for Fortran we explored during this work.

== Clusters Used in this Work <clusters>

Given that the target system for @HAWEN are clusters and supercomputers, executing the code in a system with comparable hardware was essential to accurately perform any analysis. Features such as multi-node compute and big simulations using hundreds of GB of system memory are difficult to simulate on a personal system. Furthermore, certain optimizations such as AVX512 vectorization are usually not supported on customer-grade @CPU:short:pl and not taking them into account may give a skewed perception of the behavior of the program.
...
/ DOREMI CALI v3: a cluster that is oriented towards @GPU computing. It is offered to the higher education and research community of the French region of Nouvelle-Aquitaine. It is also, like PlaFRIM, heterogenous in its configuration.

== Parallelism Tools <par-tools>

In recent years, the observation by #cite(<x-MooreLaw>, form: "prose"), which has since taken the title of "law" due to its accuracy and longevity, of the number of transistors doubling every two years in integrated circuits has been slowly proving less and less accurate. Many experts already consider Moore's Law to be a thing of the past. A recent trend in consumer electronics and in particular consumer grade @CPU:short:pl is the ever increasing number of core on a single socket. While "horizontal" scaling, typical in a server and @HPC environment has been already wide spread for decades, "vertical" scaling has only recently focused on core count due to the ever increasing difficulty of reducing the feature size and increasing the frequency of each core.

=== CUDA

@GPU:pl have established themselves as an affordable architecture for parallel computing and in particular embarrassingly parallel tasks. While originally conceived for accelerating graphic tasks, they are now used for general computing tasks. Due in part to familiarity, in part to more mature documentation, this work will concern mostly the tools provided by the NVIDIA corporation.

Regarding the development side of code targeting NVIDIA @GPU:pl, the CUDA Toolkit provides compilers, libraries and tools for developing accelerated applications. The parallel model of the @GPU is abstracted as a set of parallel and independent _streaming multiprocessors_. Each multiprocessor is responsible for scheduling a large number of threads simultaneously.

Without going into too much detail about the way the threads are scheduled and the memory is managed in a @GPU architecture, we can isolate some key take aways:

- The way threads are scheduled in _warps_ makes branching extremely costly when compared to standard @CPU:short architecture. Furthermore the absence of a sophisticated branch predictor accentuates this disadvantage.

- Memory bandwidth is extremely important in @GPU algorithm design and keeping each core "fed" with data is one of, if not the most important parts of the design. The target should be "the highest occupancy".
- The instruction set of @GPU:pl, especially when compared to x86_64, is generally much smaller than conventional processor. Being aware of native operations is important when designing algorithms or writing _kernels_. One of these native operations is the _floating point multiply add_, sometimes abbreviated with FMA.
...
Writing cross-platform code is of great interest to the team and so it was prioritized over vendor-specific solution with CUDA code being explored only when vendor-specific solutions where already inherently required.
...
// TODO: add an image about elmination trees and explain it in the context of sparse solvers
...
The GNU Fortran compiler is part of the @GCC:long and is widely used in the open-source community. Support for recent Fortran standards is not complete, but the performance and the diagnostic information are excellent. `gfortran` doesn't support yet offloading to @GPU:short:pl and only recently has included support for locality specifiers. Native reduction support is not yet available.
