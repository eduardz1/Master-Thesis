...
- In recent years @GPU:pl design has been optimized for Machine Learning tasks, where lower precision is acceptable (think for example of @LLM:pl inference). This results in more silicon dedicated on the die to these operations which often end up order of magnitude faster than ones at higher precision. Another thing to keep in mind regarding precision is that no support is planned for 128 bit floating point, which limits the work we can do in quadruple precision, something that is currently supported in @HAWEN.
...
The @HAWEN software is written in free form Fortran, which differs from the fixed source form used in FORTRAN77 by dropping some limitations that where in place for compatibility with physical punch cards. During the years, the PL22.3 committee, colloquially known as the J3 committee, published several revisions to the standard, which extend the language with new features. In particular interest for this work, is the native support of parallelism primitives, such as the `do concurrent` construct and the `coarray` data type.
