...
In this section we will talk about the tools and libraries used as part of the performance work on @HAWEN. The code of the library is highly parallel by design, in @par-tools we will talk about the main parallelism tools that will be relevant to our analysis. @HAWEN is mainly written in Fortran, so @fortran-sec will be dedicated as a brief introduction to the language and in particular the native support for parallelism primitives that have been introduced in recent versions of the standard. Having no reference implementation, multiple compilers are currently actively used and developed for the language, in @compilers we will briefly mention the ones most relevant to our work. As we saw, the code solve a very sparse large-scale linear problem, a specialized solver is used for these kinds of problems, which would be unreasonably expensive for standard dense linear solvers, @sparse-solvers will cover two of them that we chose to compare against one another. @profiling will give an overview at the current state of profiling for Fortran @MPI programs and the criteria used to choose a suitable one for this work. Likewise, @clusters will serve as a general overview of the clusters used in this work to run the aforementioned benchmarks.

== Parallelism Tools <par-tools>

In recent years, the observation by #cite(<x-MooreLaw>, form: "prose"), which has since taken the title of "law" due to its accuracy and longevity, of the number of transistors doubling every two years in integrated circuits has been slowly proving less and less accurate. Many experts, such as NVIDIA's CEO Jensen Huang @x-JensenMooreLawDead, already consider Moore's Law to be a thing of the past. A recent trend in consumer electronics and in particular consumer grade @CPU:short:pl is the ever increasing number of core on a single socket. While _horizontal scaling_ #footnote[The process of increasing the computational resources by increasing the number of server nodes.], typical in a server and @HPC environment has been already wide spread for decades, _vertical scaling_ #footnote[The process of increasing the capabilities of a single system by using newer/more powerful processor, more/newer/faster RAM, etc.] has only recently focused on core count due to the ever increasing difficulty of reducing the feature size and increasing the frequency of each core. Nonetheless, work in the integration of vertical and horizontal scaling compute has been ongoing for decades @x-EarlyMPIOpenMP and concerns from #cite(<x-LessEarlyMPIOpenMP>, form: "prose") about thermal limitations of multi-core processor are nowadays mostly resolved.

More recently, extending similar paradigms to specialized accelerators has gained more traction, as detailed in the work of #cite(<x-RecentMPIOpenMPGPU>, form: "prose"). @GPU:pl have established themselves as an affordable architecture for parallel computing and in particular embarrassingly parallel tasks. While originally conceived for accelerating graphic tasks, they are now used for general computing tasks. Due in part to familiarity, in part to more mature documentation, this work will concern mostly the tools provided by the NVIDIA corporation.
...
In the context of @HAWEN, @MPI is used to split the mesh in smaller sections using the METIS partitioner, developed by #cite(<x-METIS>, form: "prose").
...
/ DOREMI CALI v3: is a cluster that is oriented towards @GPU computing. It is offered to the higher education and research community of the French region of Nouvelle-Aquitaine. It is also, like PlaFRIM, heterogenous in its configuration.