...
When developing code targeting NVIDIA @GPU:pl, the CUDA Toolkit provides compilers, libraries and tools for developing accelerated applications. These compilers enable the developer to write code that is either compiled with a traditional host compiler, like the ones from the  @GCC, for the specified architecture or compiled to an intermediate representation, similar to that of @LLVM, called @PTX. This approach enables each program to both include an optimized binary for a specific  @GPU architecture and at the same time take advantage of the @VM @ISA to @JIT compile the code for different architectures @x-PTX. The parallel model of the @GPU is abstracted as a set of parallel and independent compute units with shared local memory and some kind of synchronization mechanism (_streaming multiprocessors_). Each multiprocessor is responsible for scheduling a large number of threads simultaneously. @GPU:pl employ the @SIMT execution model where multiple threads operate in groups on multiple @SIMD instruction streams. @GPU:pl are optimized for massively parallel tasks and are comprised of a large number of smaller, more specialized cores, compared to the ones found in @CPU:pl. In @cpu-v-gpu-arch-fig we can see a schematic representation of the differences between the @CPU and @GPU architectures.
...
- In recent years @GPU:pl designs have been optimized for Machine Learning tasks, where lower precision can be acceptable (think for example of @LLM:pl inference @x-EfficientLLMs). This results in more silicon dedicated on the die to these operations which often end up order of magnitude faster than ones at higher precision. Another thing to keep in mind regarding precision is that no support is planned for 128 bit floating point, which limits the work we can do in quadruple precision, something that is currently supported in @HAWEN.
...
Fortran remains a very popular language for scientific computing, especially in the field of numerical simulations. It has become a staple in @HPC due to its focus on performance whilst maintaining high level primitives for math operations and the extensive support for numerical libraries and tools. @x-ModernFortran

The @HAWEN software is written in free form @x-IntelFreeForm Fortran, which differs from the fixed source form @x-IntelFixForm used in FORTRAN77 by dropping some limitations that where in place for compatibility with physical punch cards. During the years, the PL22.3 committee, colloquially known as the J3 committee, published several revisions to the standard, which extend the language with new features. In particular interest for this work, is the native support of parallelism primitives, such as the `do concurrent` construct and the `coarray` data type.

Compared to other languages such as C++, the various standards are usually not fully supported in their entirety by all compilers @x-CompilerSupportModernFortran @x-CompilerSupportCPP, extensions to the language are commonplace and backwards compatibility has been historically mostly ensured (with some exceptions, like the recent breaking change in the standard 2023 @x-LLVMFortran202X) so it does not make sense to discuss having as a target a specific revision, a good discussion related to the status of the standard enforcement in different compiler can be found on the CMake repository issue tracker @x-CMakeIssueFortranStd. The software does currently implement features from the Fortran 2008 standard but does not conform to it strictly, making use of non-standard intrinsics and compiler extensions. Particular care will, therefore, be given to the support status of each language feature by the different compilers.
...
Fortran Coarrays where introduced at first as extension to the Fortran standard in the Cray compiler and later standardized in Fortran 2008. They provide a native mean for @SPMD programming in Fortran. #cite(<x-MPIvsCoarrays>, form: "prose") compare the performance of Coarrays with @MPI and found consistent and significant lower performance compared to @MPI. For this reason, we have decided not to focus on Coarrays in this work.

== Compilers <compilers>

Being Fortran a very old language with no reference implementation, during the years, many compilers have emerged, in this section will be listed some of the more relevant ones explored during our work on @HAWEN:short.

=== NVIDIA Fortran Compiler <nvfortran>

NVIDIA's Fortran compiler, formerly known as PGI Fortran, is part of the NVIDIA @HPC @SDK, which includes also C and C++ compilers, libraries, and tools for high-performance computing. It officially supports Fortran 2003 but also includes several features from later standards, in particular, for our work, we have focused on its excellent support of Fortran's standard parallelism features in the form of the `do concurrent` construct. `nvfortran` supports out of the box locality specifiers, `local` and `shared`, and `reduce` annotations for concurrent loops.
...
- #link("https://forums.developer.nvidia.com/t/why-do-i-keep-getting-managed-attribute-mismatch-in-my-routine-calls/338526/4")[TPR\#37570] @x-TPR37570: a logic conflict between the `do concurrent` device offloading  and OpenMP that led to incorrect propagation of the `managed` attribute
...
The GNU Fortran compiler is part of the @GCC:long and is widely used in the open-source community. Support for recent Fortran standards is not complete, but the performance and the diagnostic information are excellent. `gfortran` does not support yet offloading to @GPU:short:pl out of the box but can be enabled by compiling the compiler with a series of flags that enable @PTX code generation. Support for locality specifiers in `do concurrent` loops has only been added recently and is still incomplete, native parallel reduction support with the `reduce` clause in Fortran is also not yet available. This is the default compiler used in our work.

=== Intel Fortran Compiler <ifx>

Intel also provides a series of @HPC tools specifically designed for their @CPU:short:pl. Other than the Fortran compiler, Intel provides an implementation of @BLAS, MKL. A new version of the compiler, `ifx`, has been released recently, replacing the old `ifort` compiler. The new compiler is based on #link(<flang>)[LLVM Flang]. While @HAWEN currently compiles and runs correctly with it, it's relevance stems mostly from the specific optimizations they offer on Intel @CPU:short:pl and @GPU:short:pl. In this work we will mostly target AMD @CPU:short:pl and NVIDIA @GPU:short:pl instead.
...
/ Direct methods: where the sparse matrix is first factorized and the solution is then computed directly. This approach generally works for any matrix without needing any preconditioning of the data. The memory requirement is still relatively high due to the storage requirement of the factors. The precision is defined by that of the machine. After a costly analysis and factorization phases, the actual solving is very inexpensive, making it suitable for multiple right-hand sides. This is the approach that is used in @HAWEN.

/ Iterative methods: generally faster for large problems and a single right-hand side, require problem-specific preconditioning of the data, which means that convergence is not always ensured. The accuracy is limited to the square root of the machine precision. The memory requirement is much lower.

#figure(
  placement: top,
  image("../resources/imgs/A_spy_plot.svg"),
  caption: [Example of a relatively small matrix generated by @HDG:short in HAWEN. This matrix was generated with a mesh of 25 thousand cells and polynomial basis of order 1. Only the upper triangle is saved in the @COO:short representation, given that the matrix is symmetric.]
    + context {
      if not state("in-outline").get() { linebreak(justify: true) }
    },
) <a-spy>
...
The @MUMPS is a free and open-source direct sparse solver that offers a wide range of features, the innovation is led by research, with 13 PhD theses from its inception in Toulouse in 1996 @x-MUMPS. @MUMPS has become a staple in its field and is used worldwide by industrials and academics. The software offers great numerical robustness for the factorization even of ill-conditioned matrices, it is high performant (with native @MPI and OpenMP integration) and is highly efficient in handling multiple right-hand sides.
...
In recent years the popularity of machine learning and in particular @LLM:pl, has seen an ever larger focus by @GPU manufacturer in natively supporting low precision arithmetic at the hardware level. It is often the case that performing more operations in lower precision leads to better performance when compared to less operations in higher precision on @GPU. Being aware of this trend and taking advantage of lower precision arithmetic in modern accelerators is key in designing efficient @GPU:short algorithms. This same reasoning also partially applies on @CPU:short when taking advantage of @SIMD vectorization.
...
Before starting to write any code, it is important to understand the performance bottlenecks of the codebase. Some logging was already present in @HAWEN, so we had an idea on where the time was spent, but the granularity of the logging was not sufficient to pinpoint the exact functions that were taking the most time. In @prof-overview we will give an overview of the different profilers used in this work while in @tau-sect we will focus specifically on the TAU Performance System, which has been used extensively at the start of this project.

=== Overview of Profiling Tools Used in this Report <prof-overview>
